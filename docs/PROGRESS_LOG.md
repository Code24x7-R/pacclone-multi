...
### [FEATURE] Server-side leaderboard + moderator role
- Goal: move the high-score leaderboard to the server so it persists across clients and restarts, and add a moderator role to chat for admin tasks (reset the leaderboard now; extensible to future tasks).
- Server (`src/leaderboard.js`, pure UMD module): `createEntry`, `insertScore` (sort desc, cap 10, stable alpha tie-break), `resetLeaderboard`, `sanitizeEntries` (validates a board loaded from disk). No I/O in the module — server.js owns disk access — so the rules are unit-testable. 17 unit tests in `tests/server/leaderboard.test.js`.
- Server (`server.js`): leaderboard state + `loadLeaderboard`/`saveLeaderboard` (persists to `data/leaderboard.json`, best-effort so a read-only fs can't crash the game). Authoritative scoring — the server records each eliminated player's score at death and each survivor's score at match end; clients never submit scores. New messages: `getLeaderboard` (C→S), `leaderboard` (S→C broadcast), `resetLeaderboard` (C→S, moderator-gated). File path overridable via `PACCLONE_LEADERBOARD_FILE` for tests.
- Moderator role: first player to join an empty lobby becomes moderator; role transfers to the next-oldest player on departure and persists across reconnects (by token) and post-match rebuilds. Reported in `lobbyState.moderatorId`. Reset via `/resetleaderboard` chat command OR the `resetLeaderboard` message — both gated server-side (`isModerator`), so a non-moderator can't trigger a reset by faking either. A reset clears the board, persists, broadcasts the empty board, and posts a System chat notice.
- Client (`index.html`): leaderboard is now server-driven — `leaderboard` renders the board and evaluates the high-score overlay from the server's data (captures `previousTopScore` on `playerAssigned` to detect a genuinely new #1). Dropped localStorage score insertion. Added a moderator-only Reset button + MOD badge next to the moderator's name. Requests the leaderboard on every (re)join.
- 3 leaderboard integration tests (empty board, join delivery, score recording + persistence) and 6 moderator integration tests (role assignment, transfer on leave, non-mod not flagged; reset via chat command, non-mod command falls through to chat, reset via message + non-mod rejection). All 501 tests pass, 0 lint errors.
- Files changed: `src/leaderboard.js` (new), `server.js`, `index.html`, `tests/server/leaderboard.test.js` (new), `tests/integration/leaderboard.test.js` (new), `tests/integration/moderator.test.js` (new), `.gitignore`, `docs/Architecture.md`, `docs/PLAN.md`, `docs/PROGRESS_LOG.md`.
