...
| :--- | :--- | :--- | :--- | :--- | :--- |
| B-010 | Player stuck after level 2 start — can only move on tiles where pellets were eaten, cannot move on tiles with pellets | `isWall(nextX, player.y)` called without the `maze` argument, so it fell back to the default static `MAZE` instead of using `currentMaze`. On level 2+, the procedurally generated maze differs from the static one, so wall checks used the wrong grid. Player could only move where the static maze had walkable tiles. | Pass `currentMaze` as the third argument to both `isWall` calls in the player movement block. 5 new regression tests. | 2026-08-10 | — |
